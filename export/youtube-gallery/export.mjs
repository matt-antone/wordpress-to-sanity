...
const exportFile = `${exportDir}/youtubes.ndjson`;

async function exportType() {
  const data = await fetchGraphql(query);
  const transformedData = await transformNodes(data.youtubes.nodes);
  try {
    fs.readdirSync(exportDir);
  } catch (e) {
    fs.mkdirSync(exportDir);
  }
  console.log(`Exporting ${transformedData.length} youtubes to ${exportFile}`);
